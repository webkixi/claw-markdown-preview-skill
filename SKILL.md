...
```bash
# 单实例清理：只终止上一次本技能启动的预览实例（通过 PID 文件精确定位），不误伤其他进程（含其他 markdown 预览）
if [ -f /tmp/claw-markdown-preview.pid ]; then
  OLD_PID=$(cat /tmp/claw-markdown-preview.pid 2>/dev/null)
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
    kill "$OLD_PID" 2>/dev/null
  fi
  rm -f /tmp/claw-markdown-preview.pid
fi
